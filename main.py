...
parser = argparse.ArgumentParser(prog="Hangman", description="A small game of hangman")
parser.add_argument(
    "-f",
    "--file",
    help="File containing wordbank, by default will use the provided WORDBANK.txt",
)
...
def mainMenu():
    ...
    while not flag:
        ...
        try:
            choice = input(
                "What would you like to do? (Input number corresponding to main menu)\n>> "
            )
...
def playGame(wordbank):
    ...
    while tryAgain:
        ...
        while (maskedWord != chosenWord) and incorrectGuesses < 7:
            ...
            maskedWord, incorrectGuesses, score = updateWord(
                chosenWord, maskedWord, guess, incorrectGuesses, score
            )
